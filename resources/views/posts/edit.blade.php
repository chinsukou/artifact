        ...
        <form action="/posts/{{ $post->id }}" method="POST" enctype="multipart/form-data">
          ...
          <div class='my-2'>
            ...
            <div>
              <select class='font-bold border-none w-full' name="post[category_id]" >
                <option selected value="{{ $post->category->id }}">カテゴリー：{{ $post->category->name }}</option>
            ...
            <div class=''>
              <select class='font-bold border-none w-full' name="post[difficulty_id]">
                <option selected value="{{ $post->difficulty->id }}">教材の難易度：{{ $post->difficulty->name }}</option>
          ...
          <div class="">
            <textarea class='border-none border-top resize-none h-1/3 w-full' name="post[body]"
              placeholder="おすすめしたい教材.当時の自分のレベルなどを投稿してください.">{{ $post->body }}</textarea>
            <p class="body_error" style="color:red">{{ $errors->first('post.body') }}</p>
          </div>
      </div>
      <div class='flex justify-end p-4'>
        <input class='rounded bg-blue-500 hover:bg-blue-700 text-white w-fit' type="submit" value="編集を終わる" />
      </div>
      </form>
